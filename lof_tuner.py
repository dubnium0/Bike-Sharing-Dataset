    ...
    def visualise(self):
        ...
        for i, v in tqdm.tqdm(enumerate(self.collector)):
            ...
            if Kopt == self.tuned_params['k'] and contamination == self.tuned_params['c']:
                ax[1, 1].scatter(self.data[:, 0], self.data[:, 1], facecolors='none', s=1000 * log_lof,
                                 edgecolors='darkgray')

            ax[1, 1].scatter(np.ma.masked_where(log_lof < thresh, self.data[:, 0]),
                             np.ma.masked_where(log_lof < thresh, self.data[:, 1]), c='orange', s=5.0)
            ax[1, 1].scatter(np.ma.masked_where(log_lof > thresh, self.data[:, 0]),
                             np.ma.masked_where(log_lof > thresh, self.data[:, 1]), c='green', s=5.0)
